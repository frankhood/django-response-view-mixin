...
class PDFResponseMixin:
    ...
    def render_to_response(self, context, **response_kwargs):
        file_path = f"{self.get_file_dir()}{os.sep}{self.get_file_name()}"
        with open(file_path, "rb") as pdf:
            response = HttpResponse(pdf.read(), content_type=self.file_mimetype)
            response["Content-Disposition"] = f"inline;filename={self.get_file_name()}"
            return response
    ...
    def get_file_docroot(self, **kwargs):
        if self.use_private_storage:
            if not hasattr(settings, "PRIVATE_ROOT"):
                logger.error(
                    "Cannot use PRIVATE_FILE_SYSTEM without settings PRIVATE_ROOT"
                )
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        try:
            response["Content-Disposition"] = ('attachment; filename="{}"' "").format(
                response.get(
                    "Content-Disposition",
                    os.path.split(self.get_file_path(**context))[-1],
                )
            )
        except Exception:
            logger.exception("")
