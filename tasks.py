...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-response-mixin-view runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-response-mixin-view.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_response_mixin_view")
...
def lint(c):
    ...
    """
    c.run("flake8 django-response-mixin-view tests")
...
def release(c, bumpsize=''):
    ...
    import django_response_mixin_view
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_response_mixin_view.__version__))
    c.run("git push --tags")
